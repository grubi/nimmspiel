<script>
	import Matches from './Matches.svelte';
	export let setup, initialCount, takeMax, computerBegins;
	let count = initialCount;
	let playerTake = null;
	let computerTake = null;
	let interactive = true;
	let gameOver = false;
	let playerWins = false;
	const delayTime = 2000;

	// Timer
	let timerId = null;
	function cleanUpTimer() {
		if(timerId) {
			clearTimeout(timerId);
		}
	}
	import { onDestroy } from 'svelte';
	onDestroy(cleanUpTimer);
	
	if(computerBegins) {
		interactive = false;
		if(count==1) {
			gameOver = true;
		}
		else {
			timerId = setTimeout(computerMove, delayTime);
		}
	}
	else {
		if(count==1) {
			interactive = false;
			playerTake = 0;
			gameOver = true;
			playerWins = true;
		}
	}

	function handleTake(event) {
		playerTake = event.detail.take;
		count -= playerTake;
		interactive = false;
		if(count===1) {
			gameOver = true;
			playerWins = true;
		}
		else {
			timerId = setTimeout(computerMove, delayTime);
		}
	}

	function computerMove() {
		let take = (count-1) % (takeMax+1);
		if(take===0) {
			take = Math.floor(Math.random()*takeMax)+1;
		}
		computerTake = take;
		timerId = setTimeout(finalizeComputerMove, delayTime);
	}

	function finalizeComputerMove() {
		count -= computerTake;
		if(count===1) {
			gameOver = true;
		}
		else {
			computerTake = null;
			playerTake = null;
			interactive = true;
		}
	}
</script>

<p>
	<button on:click={() => setup=true}>Neues Spiel</button>
</p>
<p>
	Es {count==1 ? 'ist' : 'sind'} {count} Hölzchen da.<br>
	Du musst mindestens eins nehmen, aber höchstens {takeMax}.<br>
	Wer das letzte nehmen muss, verliert.
</p>
<p>
	<Matches on:take={handleTake} {count} {takeMax} bind:interactive displayTake={gameOver ? null: computerTake} />
</p>
{#if playerTake!=null}
	<div style="color: blue">
	{#if playerTake!=0}
		<p><strong>Du nimmst {playerTake}.</strong></p>
	{/if}
	{#if playerWins}
		<h1>Du hast gewonnen!</h1>
	{/if}
	</div>
{/if}
{#if computerTake || (computerBegins && count===initialCount)}
	<div style="color: red">
		{#if computerTake}
			<p><strong style="color: red">Ich nehme {computerTake}.</strong></p>
		{:else}
			<p><strong style="color: red">Ich denke nach...</strong></p>
		{/if}
		{#if gameOver}
			<h1>Ich habe gewonnen!</h1>
		{/if}
	</div>
{/if}
{#if !(playerTake || computerTake) && interactive}
	<p>
		Klicke auf das Hölzchen, das du nehmen willst.<br>
		Alle rechts davon werden ebenfalls genommen.
	</p>
{/if}
